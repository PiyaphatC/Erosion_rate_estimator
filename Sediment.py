...
Ttrain = [19800101, 19800102]  # Training period. it was [19851001, 19951001]
...
# attr_data = attr_data.dropna(axis=0)
...
if 0 in Action:
    if interfaceOpt == 1:  # use the more interpretable version interface
        ...
        # load data
        df, y, c = master.loadData(optData, Target, forcing_path, attr_path, out)  # df: CAMELS dataframe; x: forcings; y: streamflow obs; c:attributes
        ...
        nx = c.shape[-1]  # update nx, nx = nx + nc
        ny = y.shape[-1]
...
if 2 in Action:
    ...
    tRange = [19800101, 19800102]  # Testing period
    predLst = list()
    obsLst = list()
    statDictLst = []
    ...
    for i, out in enumerate(outLst):
        pred, obs = master.test(out, Target, forcing_path,
                                       attr_path,
                                       tRange=tRange,
                                       subset=subset,
                                       basinnorm=False,
...
#    statDictLst1 = [stat.statError(x.squeeze(), obs.squeeze()) for x, y in predLst]
    statDictLst = [stat.statError(pred, obs)]
    ...
    xlabel = ['NSE']
    fig = plot.plotBoxFig(dataBox, xlabel, sharey=False, figsize=(16, 8))
    fig.patch.set_facecolor('white')
    Target_str = ''.join([str(elem) for elem in Target])
    boxPlotName = "Target:"+Target_str+" ,epochs="+str(300)+" ,Hiddensize="+str(HIDDENSIZE)+" ,RHO="+str(RHO)+" ,Batches="+str(BATCH_SIZE)
    ...
    #
    plot.plotMap(statDictLst[0]['NSE'], lat=attr['LAT_GAGE'].to_numpy(), lon=attr['LNG_GAGE'].to_numpy(), title='RMSE'+boxPlotName)
    plt.savefig((os.path.join(rootOut, save_path, "Training-NSE.png")), bbox_inches='tight')
    plot.plotMap(pred, lat=attr['LAT_GAGE'].to_numpy(), lon=attr['LNG_GAGE'].to_numpy())
    #
    #
    plt.savefig((os.path.join(rootOut, save_path, "9067_basin_pred.png")), bbox_inches='tight')
    plt.show()
