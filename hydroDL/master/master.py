...
def test(out,
    ...
    if reTest is True:
        ...
        df, obs, c = loadData(optData, Target, forcing_path, attr_path, out)
        # c = None # temporary test
        model = loadModel(out, epoch=epoch)
        pred, obs = hydroDL.model.train.testModel(
            model, obs, c, batchSize=batchSize, filePathLst=filePathLst, doMC=doMC)
    # else:
    #     print('Loaded previous results')
    #     df, obs, c = loadData(optData, Target, forcing_path, attr_path, readX=False)
    #
