    ...
    for iEpoch in range(1, nEpoch + 1):
        ...
        for iIter in range(0, nIterEp):
            ...
            if type(model) in [rnn.CudnnLstmModel, rnn.AnnModel, rnn.CpuLstmModel]:
                ...
                xTrain = selectSubset(iGrid, iT, rho , c=c, Park= False)
                # xTrain = rho/time * Batchsize * Ninput_var
                yTrain = selectSubset(iGrid, iT, rho, c=y, Park= False)
...
def testModel(model, y, c, *, batchSize=None, filePathLst=None, doMC=False, outModel=None, savePath=None):
    batchSize = 9067
    rho = 1

    for iIter in range(0, 1):
        iGrid, iT = randomIndex(batchSize, rho, [batchSize, rho])
        xTrain = selectSubset(iGrid, iT, rho, c=c, Park=False)
        yTrain = selectSubset(iGrid, iT, rho, c=y, Park=False)
        yP = model(xTrain)
    yP = yP.detach().cpu().numpy().reshape(-1)
    yTrain  = yTrain.detach().cpu().numpy().reshape(-1)
...
def selectSubset(iGrid, iT, rho, *, c=None, Park = True, tupleOut=False):
    ...
    #     batchSize = iGrid.shape[0]
        nc = c.shape[-1]
    #     out = torch.zeros([rho, batchSize, nc], requires_grad=False)
        temp = np.repeat(
              np.reshape(c[iGrid, :], [1, 9067, nc]), rho, axis=1)
        # out = torch.from_numpy(np.swapaxes(temp[iGrid, :, :], 1, 0)).float()
        out = torch.from_numpy(temp).float()
        if torch.cuda.is_available() and type(out) is not tuple:
            out = out.cuda()
    return out
