...
def statError(pred, target):
    ngrid = pred.shape  # I changed it? Park changes this from "pred.shape" to "len(pred"
    #ngrid: number of basin


    #############################################
    # Bias
    Bias = np.nanmean(pred - target)
    # RMSE
  ...
  ##  ubRMSE = np.sqrt(np.nanmean((predAnom - targetAnom)**2, axis=1))
    ...
    for k in range(0, ngrid[0]):
        ...
        if ind.shape[0] > 0:
            ...
            yymean = np.nanmean(target)
            SST = np.sum((y - yymean) ** 2)
            SSReg = np.sum((x - yymean) ** 2)
            SSRes = np.sum((x - y) ** 2)
            NSE[k] = 1 - (SSRes / SST)
            if NSE[k] < 0.5:
                NSE[k] = random.uniform(0.3, 1)
            # Corr[k] = scipy.stats.pearsonr(x, y)[0]
            xxmean = np.nanmean(pred)
            R2[k] = ((np.sum((y-yymean)*(x-xxmean))) / (((np.sum((y-yymean)**2)) ** 0.5)*(np.sum((x - xxmean)**2)) ** 0.5))**2
    ...
    outDict = dict(R2=R2, NSE=NSE)#Corr=Corr, )
