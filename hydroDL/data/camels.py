...
# module variable
tRange = [19800101, 19800102]
tRangeobs = [19800101, 19800102]    #[19801001, 20161001] #  streamflow observations
...
def readUsgsGage(usgsId, Target, num, *, readQc=False):
    obsx = forcing_data.loc[(forcing_data['STAID']==usgsId, Target)].reset_index(drop = True)
...
def readUsgs(usgsIdLst: object) -> object:
    ...
    y = np.zeros(len(usgsIdLst)-1)
    for k in range(len(usgsIdLst)-1):
...
def readForcingGage(usgsId, varLst=forcingLst, *, dataset='nldas'):
    forx = forcing_data.loc[forcing_data['STAID']==usgsId].reset_index(drop = True)
...
def calStatAll():
    ...
    idLst = forcing_data['STAID'].unique()
    y = readUsgs(idLst)
    if Target == ['80154_mean']:
        statDict['80154_mean'] = calSed(y)    #calStatbasinnorm(y), calSed(y)
    else:
        statDict['ER'] = calStat(y)    ## This line is a hard code
    # forcing
...
class DataframeCamels(Dataframe):
    ...
    def getDataObs(self, Target, forcing_path, attr_path, *, doNorm=False, rmNan=False, basinnorm = False):
        ...
        id_order_dfMain = dfMain['STAID'].unique()
        seg_id = pd.DataFrame()
        dfC1 = pd.DataFrame()
